...
#ps0 <- subset_samples(ps0, year == 2010)

ps0 <- subset_taxa(ps0, Kingdom %in% c("Bacteria", "Archaea"))
...
```{r, warning=FALSE, fig.fullwidth=TRUE, fig.width=10}
plot_bar2(ps4n, x = "stn.name", y = "Abundance", fill = "Phylum") + facet_grid(cruise~., scales = "free_x", space = "free_x")
```


```{r, warning=FALSE, fig.fullwidth=TRUE, fig.width=10}
eyja <- subset_samples(ps4n, transect %in% c("EY") )
plot_bar2(eyja, x = "Sample", y = "Abundance", fill = "Phylum") + facet_grid(stn~depth, scales = "free_x", space = "free_x")
```
